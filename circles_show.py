...
tr_stims[0][0].index = inde
tr_stims[1] = [dots[0:10], 1]
tr_stims[1][0].index = inde
tr_stims[2] = [dots[10:20], 1]
tr_stims[2][0].index = inde
tr_stims[3] = [dots[10:20], 2]
tr_stims[3][0].index = inde
tr_stims[4] = [dots[20:30], 2]
tr_stims[4][0].index = inde
tr_stims[5] = [dots[20:30], 0]
tr_stims[5][0].index = inde
# 随机刺激样本
...
for i in range(len(stims)):
    ...
    result['average_x'].append(stims[i][0]['x'][9])
    # 刺激
    for j in range(9):
...
with open("exp_data\\%s.csv" % (result['name']+time.strftime("%H-%M-%S")), 'a') as exp_data:
    exp_data.write(
        'num' + ',' + 'name' + ',' + 'age' + ',' + 'sex' + ',' + 'average_x' + ',' + 'cover' + ','+'estimate_size'+'\n')
